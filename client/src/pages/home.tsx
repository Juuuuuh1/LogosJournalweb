...
export default function Home() {
  ...
  const generateImage = async (style: 'artwork' | 'sketch' = 'artwork') => {
    ...
    const prompt = style === 'sketch' 
      ? `Create a hand-drawn sketch that captures the philosophical essence and personal reflections from these thoughts. Style: ${selectedSketchStyle}. The image should be either black and white line art or colored sketch showing a contemplative scene, character in thoughtful pose, or symbolic representation of inner reflection. Use clean, expressive lines and thoughtful composition. Ensure clean, sharp edges with no noise, artifacts, or visual disturbances around the borders. The entire image should have a polished, professional appearance with crisp details and smooth edge transitions. Prefer pure visual storytelling without text. Only include speech bubbles or text if absolutely essential for conveying the philosophical meaning, and if you do, use meaningful quotes or key insights from the reflection content - never placeholder text. Focus primarily on visual storytelling through drawing techniques. Personal reflections: ${personalizedContent.substring(0, 400)}`
